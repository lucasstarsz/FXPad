...
public class RTFXController {
    ...
    private void cantOpenFile(String fileName) {
        ...
        cantOpenFileAlert.initOwner(RTFXMain.getStage());
    ...
    private void saveFileAs() throws IOException {
        ...
        File file = chooser.showSaveDialog(RTFXMain.getStage());
        if (file != null) {
            write(file);
            setCurrentFile(file);
        }
    ...
    private void openFile() throws IOException {
        ...
        File file = chooser.showOpenDialog(RTFXMain.getStage());
    ...
    private void open(File file) throws IOException {
        textArea.replaceText(Files.readString(file.toPath()));
    }
    ...
    private void shift(int shiftAmount) {
        System.out.println(shiftAmount - 1 > 0);
        if (textArea.selectedTextProperty().getValue().isEmpty()) {
            Map.Entry<String, int[]> currLine = getCurrentLine(textArea);

            if (currLine.getKey().startsWith("\t") || shiftAmount > 0) {
                if (shiftAmount > 0) {
                    textArea.replaceText(currLine.getValue()[0], currLine.getValue()[1], (shiftAmount - 1 > 0 ? "\t".repeat(shiftAmount - 1) : '\t') + currLine.getKey());
...
//            textArea.selectLine();
//            textArea.selectRange(selectionStart + firstTabMovement, selectionEnd + totalTabMovement - 1);
        }
    }

    private static Map.Entry<String, int[]> getCurrentLine(StyleClassedTextArea tx) {
